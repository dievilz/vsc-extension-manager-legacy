...
import { ExtensionTreeProvider, ExtensionStatus, } from "./ExtensionTreeProvider.js";
import * as jsonc from "jsonc-parser";
// Global tree provider instance
let extensionTreeProvider;
// Helper function to get CLI path
function getCLIPath() {
    const config = vscode.workspace.getConfiguration("extensionManager");
    let cliCommand = config.get("cliCommand") || "auto";
    ...
    if (cliCommand === "auto") {
        try {
            ...
            if (fs.existsSync(binDir)) {
                ...
                let bestMatch = "";
                const candidates = files.filter((f) => !f.startsWith(".") && !f.includes("tunnel"));
...
async function installExtension(extensionId, cliPath, outputChannel) {
    return new Promise((resolve) => {
        const command = `${cliPath} --install-extension ${extensionId}`;
        cp.exec(command, (error, stdout, stderr) => {
            if (error) {
                outputChannel.appendLine(`Error installing ${extensionId}: ${error.message}`);
                if (stderr)
...
export function activate(context) {
    ...
    function getSettingsPath() {
        ...
        if (process.platform === "darwin") {
            ...
            settingsPath = path.join(context.globalStorageUri.fsPath, "../../settings.json");
        }
    ...
    const loadFileCommand = vscode.commands.registerCommand("vscode-extension-manager.loadFile", async () => {
        ...
        if (uri && uri[0]) {
            try {
                ...
                vscode.window.showInformationMessage(`Loaded ${extensions.length} extensions${data.settings ? " and settings" : ""}.`);
            }
            catch (error) {
    ...
    const installSelectedCommand = vscode.commands.registerCommand("vscode-extension-manager.installSelected", async () => {
        ...
        const outputChannel = vscode.window.createOutputChannel("Extension Manager");
        outputChannel.show();
        ...
        await vscode.window.withProgress({
                ...
                try {
                    ...
                    if (fs.existsSync(settingsPath)) {
                        try {
                            ...
                            outputChannel.appendLine(`Warning: Could not parse current settings.json: ${e}`);
                        }
                    }
                    ...
                    extensionTreeProvider.updateExtensionStatus("settings", ExtensionStatus.Success);
                    outputChannel.appendLine(`Successfully applied settings.`);
                }
                catch (e) {
                    extensionTreeProvider.updateExtensionStatus("settings", ExtensionStatus.Failed, e.message);
                    outputChannel.appendLine(`Failed to apply settings: ${e.message}`);
                }
            }
            ...
            for (let i = 0; i < total; i++) {
                ...
                if (vscode.extensions.getExtension(extensionId)) {
                    extensionTreeProvider.updateExtensionStatus(extensionId, ExtensionStatus.AlreadyInstalled);
                    continue;
                }
                progress.report({
                ...
                extensionTreeProvider.updateExtensionStatus(extensionId, ExtensionStatus.Installing);
                outputChannel.appendLine(`Installing ${extensionId}...`);
                const result = await installExtension(extensionId, cliPath, outputChannel);
                if (result.success) {
                    installedCount++;
                    extensionTreeProvider.updateExtensionStatus(extensionId, ExtensionStatus.Success);
                    outputChannel.appendLine(`Successfully installed ${extensionId}`);
                }
                else {
                    errorCount++;
                    extensionTreeProvider.updateExtensionStatus(extensionId, ExtensionStatus.Failed, result.error);
                    outputChannel.appendLine(`Failed to install ${extensionId}`);
                }
            }
            vscode.window.showInformationMessage(`Process complete. Installed: ${installedCount} extensions. Settings applied: ${settingsItem ? "Yes" : "No"}.`);
        });
    });
    ...
    const selectAllCommand = vscode.commands.registerCommand("vscode-extension-manager.selectAll", () => {
        extensionTreeProvider.selectAll();
    });
    // Register deselectAll command
    const deselectAllCommand = vscode.commands.registerCommand("vscode-extension-manager.deselectAll", () => {
        extensionTreeProvider.deselectAll();
    });
    // Register showError command
    const showErrorCommand = vscode.commands.registerCommand("vscode-extension-manager.showError", (item) => {
        if (item.errorMessage) {
            vscode.window.showErrorMessage(`Error: ${item.errorMessage}`);
        }
    });
    context.subscriptions.push(treeView, loadFileCommand, installSelectedCommand, selectAllCommand, deselectAllCommand, showErrorCommand);
    ...
    let exportCommand = vscode.commands.registerCommand("vscode-extension-manager.exportExtensions", async () => {
        try {
            ...
            const outputChannel = vscode.window.createOutputChannel("Extension Manager Debug");
            try {
                if (fs.existsSync(settingsPath)) {
                    outputChannel.appendLine(`Found settings.json at: ${settingsPath}`);
                    const content = fs.readFileSync(settingsPath, "utf8");
                    settingsData = jsonc.parse(content);
                    outputChannel.appendLine(`Read ${Object.keys(settingsData).length} settings.`);
                }
                else {
                    outputChannel.appendLine(`WARNING: settings.json NOT found at: ${settingsPath}`);
                    vscode.window.showWarningMessage(`Could not find settings.json at: ${settingsPath}. If you are in Debug mode, this is expected (separate profile).`);
                }
            }
            catch (e) {
            ...
            if (uri) {
                ...
                vscode.window.showInformationMessage(`Successfully exported ${extensionList.length} extensions and settings to ${uri.fsPath}`);
            }
        }
        catch (error) {
    ...
    let importCommand = vscode.commands.registerCommand("vscode-extension-manager.importExtensions", async () => {
        try {
            ...
            if (uri && uri[0]) {
                ...
                const confirmation = await vscode.window.showQuickPick(["Yes", "No"], { placeHolder: `Install ${extensions.length} extensions?` });
                if (confirmation === "Yes") {
                    ...
                    const outputChannel = vscode.window.createOutputChannel("Extension Manager");
                    outputChannel.show();
                    outputChannel.appendLine(`Starting installation of ${extensionsToInstall.length} extensions...`);
                    ...
                    await vscode.window.withProgress({
                        ...
                        vscode.window.showInformationMessage(`Installation complete. Installed: ${installedCount}, Failed: ${errorCount}. Check 'Extension Manager' output for details.`);
                    });
                }
            }
        }
